...
import { fromEvent, merge, Observable, Observer, Subscription, timer } from 'rxjs'
...
export class AppComponent implements OnInit {
  ...
  resetTimer() {
    if (this.subscribe) {
      ...
      this.initTimer(new Date().getTime() + 900000)
    }
  }
  _locale: String
  _theme: String
  ...
  ngOnInit() {
    ...
    // })
    //start signal-R
    this.wSocket = this.waiterService.waiterSocket
    this.startSignalR()
    ...
    this.router.events
      ...
      .subscribe(event => this.titleService.setTitle('Biz1Book (v1.0.28) | ' + event['title']))
  ...
  socketConfigure() {
    this.waiterService.connect()
    console.log(this.wSocket)
    var wSocket = this.waiterService.waiterSocket
    wSocket.on("order:create", (order) => {
      console.log(order)
    })
  }
    ...
    if (!this.url.includes('/auth/')) {
      ...
      this.subscribe = source.subscribe(val => {
        ...
        if (start_time == endTime) {
          if (!this.url.includes("/auth/")) {
            this.subscribe.unsubscribe()
            console.log("15 mins of inactivity" + moment().format('HH:MM A'))
