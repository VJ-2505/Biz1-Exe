...
export class ReceiptComponent implements OnInit {
  ...
  paymenttypes = []
  ngOnInit(): void {
    this.Auth.getdbdata(['preferences', 'loginfo', 'printersettings', 'paymenttypes']).subscribe(data => {
...
<style>
  ...
  printreceipt() {
    ...
    <hr>
    <div ${this.customer.PhoneNo ? '' : 'hidden'} class="header">
        <h3 ${this.customer.Name ? '' : 'hidden'}>${this.customer.Name}</h3>
        <p>${this.customer.Address ? this.customer.Address + '<br>' : ''}${this.customer.City ? this.customer.City + ',' : ''
      }${this.customer.PhoneNo}</p>
    </div>
  ...
  onDateSelection(date: NgbDate, datepicker) {
    ...
    if (this.fromDate && this.toDate) {
      ...
      this.enddate = this.formatter.format(this.toDate)
      datepicker.toggle()
      this.getReceipt(0, 'next', 'all', this.formatter.format(this.fromDate), this.formatter.format(this.toDate), null)
    }
  }
